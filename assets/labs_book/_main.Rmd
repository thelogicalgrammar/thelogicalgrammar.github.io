...
# Introduction lab (week 2)

## Bits and pieces of R

Some useful shortcuts for writing markdown:

- crtl+shift+k: Render in html
- crtl+alt+i: Insert r cell
...
```{r}
posterior <- bayes.numerator / colSums(bayes.numerator)
print(posterior)
```
...
3. Suppose there is a disease that affects 0.3\% of the population, and a test that correctly identifies the disease 97% of the times (i.e. $p( \text{test positive} \mid \text{has disease} )=0.97$).
  - You took the test and it was positive. What is the probability that you have the disease?
...
3. 
    1. I basically gave everyone full points for this sub-exercise because of the initial typo. Either way, the solution would be simply an application of Bayes theorem:
    $$
...
Finally, let's _marginalize_ across categories. For each point in the space (i.e. integers between 1 and 100 inclusive) sum the probabilities of the categories that include that point.
...
# Cultural Evolution in Bayesian agents (Week 4)
...
# Rational Speech Act models (week 5)
...
# Inferring causal relations in the world (week 6)
...
# Learning a Language of Thought (week 4)
